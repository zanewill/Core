...
namespace CastleTests.OpenGenerics
{
	...
	public class ClassProxyWithTargetEmptyClassTestCase : BasePEVerifyTestCase
	{
		[Test]
		public void Can_generate_generic_and_non_generic_proxy_for_interfaces_with_same_name_one_generic_one_not()
		{
			var one = generator.CreateClassProxyWithTarget(new Empty<string>());
			var two = generator.CreateClassProxyWithTarget(new Empty());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new EmptyClass<object>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new EmptyWithBaseGenericInterfaceConstraint<GenInterfaceImpl<int>>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new EmptyWithBaseInterfaceConstraint<Empty>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new EmptyNew<int>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new EmptyStruct<int>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new EmptyVariant<object, string>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new EmptyWithBase<int>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new Empty<string>());
			var two = generator.CreateClassProxyWithTarget(new Empty<string, int>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new Empty<string>());
			var two = generator.CreateClassProxyWithTarget(new Empty<int>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(new Empty<string>());
			var two = generator.CreateClassProxyWithTarget(new Empty<string>());
		...
		{
			var one = generator.CreateClassProxyWithTarget(typeof (Empty<string>), new[] { typeof (IEmpty) }, new Empty<string>());
			Assert.True(one.GetType().IsGenericType, string.Format("Expected proxy type ({0}) to be generic", one.GetType()));
		}
		...
		public void Can_generate_generic_proxy_with_additional_interface_and_mixin()
		{
			...
			var one = generator.CreateClassProxyWithTarget(typeof (Empty<string>), new[] { typeof (ISimple) },
		...
		public void Can_generate_generic_proxy_with_mixin()
		{
			var options = new ProxyGenerationOptions();
			options.AddMixinInstance(new Empty());
			var one = generator.CreateClassProxyWithTarget(typeof (Empty<string>), new Empty<string>(), options);
			Assert.True(one.GetType().IsGenericType, string.Format("Expected proxy type ({0}) to be generic", one.GetType()));
		}
		...
		{
			var one = generator.CreateClassProxyWithTarget(typeof (Empty<string>), new[] { typeof (IEmpty<string, int>) },
			                                               new Empty<string>());
		...
		{
			var proxy = generator.CreateClassProxyWithTarget(new Empty<string>());
		...
		{
			var proxy = generator.CreateClassProxyWithTarget(new Empty<IDictionary<Func<string>, ICollection<Predicate<int>>>>());
