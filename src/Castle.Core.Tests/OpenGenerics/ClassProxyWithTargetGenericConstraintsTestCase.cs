...
namespace CastleTests.OpenGenerics
{
	...
	public class ClassProxyWithTargetGenericConstraintsTestCase : BasePEVerifyTestCase
	{
		private T ProxyFor<T>(T target) where T : class
		{
			return generator.CreateClassProxyWithTarget(target, new DoNothingInterceptor());
		}

		[Test]
		public void Method_argument_is_constrained_to_default_ctor_class_type_argument_is_contravariant()
		{
			var one = ProxyFor(new Constraint_MethodIsClassNew_Type_is_contravariant<object>());
		...
		{
			var one = ProxyFor(new Constraint_MethodIsTypeAndStruct<object>());
			one.Method<int>();
		}
		...
		{
			var one = ProxyFor(new Constraint_MethodIsTypeAndClass<object>());
			one.Method<string>();
		}
		...
		{
			var one = ProxyFor(new Constraint_MethodIsTypeAndStruct_TypeIsClass<object>());
			one.Method<int>();
		}
		...
		{
			var one = ProxyFor(new Constraint_MethodIsType<object>());
			one.Method<int>();
		}
		...
		{
			var one = ProxyFor(new Constraint_MethodIsType_TypeIsClass<object>());
			one.Method<int>();
		}
		...
		{
			var one = ProxyFor(new Constraint_Method1IsTypeStructAndMethod2<object>());
