...
namespace CastleTests.OpenGenerics
{
	...
	public class ClassProxyNoInvocationTestCase : BasePEVerifyTestCase
	{
		...
		{
			var one = ProxyFor<SimpleGeneric<object>>();
		...
		{
			var one = ProxyFor<SimpleReturnGeneric<object>>();
		...
		{
			var one = ProxyFor<SimpleGenericConstraint<object>>();
		...
		{
			var one = ProxyFor<SimpleArg<object>>();
		...
		{
			var one = ProxyFor<SimpleReturn<object>>();
		...
		{
			return generator.CreateClassProxy<T>(proxyNothing);
		}
	}
}