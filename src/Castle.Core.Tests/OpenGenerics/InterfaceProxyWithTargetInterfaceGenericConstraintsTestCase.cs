...
namespace CastleTests.OpenGenerics
{
	...
	public class InterfaceProxyWithTargetInterfaceGenericConstraintsTestCase : BasePEVerifyTestCase
	{
		private T ProxyFor<T>(T target) where T : class
		{
			return generator.CreateInterfaceProxyWithTargetInterface<T>(target, new DoNothingInterceptor());
		}

		[Test]
		public void Method_argument_is_constrained_to_default_ctor_class_type_argument_is_contravariant()
		{
			var one = ProxyFor<IConstraint_MethodIsClassNew_Type_is_contravariant<object>>(new Constraint_MethodIsClassNew_Type_is_contravariant<object>());
		...
		{
			var one = ProxyFor<IConstraint_MethodIsTypeAndStruct_TypeIsClass<object>>(new Constraint_MethodIsTypeAndStruct_TypeIsClass<object>());
			one.Method<int>();
		}
		...
		{
			var one = ProxyFor<IConstraint_Method1IsTypeStructAndMethod2<object>>(new Constraint_Method1IsTypeStructAndMethod2<object>());
