...
namespace CastleTests.OpenGenerics
{
	...
	public class ClassProxyWithTargetWithInvocationTestCase : BasePEVerifyTestCase
	{
		[Test]
		public void Generic_method_arg_on_type_and_method_has_same_name()
		{
			var one = ProxyFor(new SimpleGenericSameName<object>());
		...
		{
			var one = ProxyFor(new SimpleGenericProtectedGenericMethod<object>());
		...
		{
			var one = ProxyFor(new SimpleGenericProtectedMethod<object>());
