...
namespace CastleTests.OpenGenerics
{
	...
	public class ClassProxyEmptyClassTestCase : BasePEVerifyTestCase
	{
		[Test]
		public void Can_generate_generic_and_non_generic_proxy_for_interfaces_with_same_name_one_generic_one_not()
		{
			var one = generator.CreateClassProxy<Empty<string>>();
			var two = generator.CreateClassProxy<Empty>();
		...
		{
			var one = generator.CreateClassProxy<EmptyClass<object>>();
		...
		{
			var one = generator.CreateClassProxy<EmptyWithBaseGenericInterfaceConstraint<GenInterfaceImpl<int>>>();
		...
		{
			var one = generator.CreateClassProxy<EmptyWithBaseInterfaceConstraint<Empty>>();
		...
		{
			var one = generator.CreateClassProxy<EmptyNew<int>>();
		...
		{
			var one = generator.CreateClassProxy<EmptyStruct<int>>();
		...
		{
			var one = generator.CreateClassProxy<EmptyVariant<object, string>>();
		...
		{
			var one = generator.CreateClassProxy<EmptyWithBase<int>>();
		...
		{
			var one = generator.CreateClassProxy<Empty<string>>();
			var two = generator.CreateClassProxy<Empty<string, int>>();
		...
		{
			var one = generator.CreateClassProxy<Empty<string>>();
			var two = generator.CreateClassProxy<Empty<int>>();
		...
		{
			var one = generator.CreateClassProxy<Empty<string>>();
			var two = generator.CreateClassProxy<Empty<string>>();
		...
		{
			var one = generator.CreateClassProxy(typeof (Empty<string>), new[] { typeof (IEmpty) });
			Assert.True(one.GetType().IsGenericType, string.Format("Expected proxy type ({0}) to be generic", one.GetType()));
		}
		...
		public void Can_generate_generic_proxy_with_additional_interface_and_mixin()
		{
			...
			var one = generator.CreateClassProxy(typeof (Empty<string>), new[] { typeof (ISimple) }, options);
			Assert.True(one.GetType().IsGenericType, string.Format("Expected proxy type ({0}) to be generic", one.GetType()));
		}
		...
		public void Can_generate_generic_proxy_with_mixin()
		{
			var options = new ProxyGenerationOptions();
			options.AddMixinInstance(new Empty());
			var one = generator.CreateClassProxy(typeof (Empty<string>), options);
			Assert.True(one.GetType().IsGenericType, string.Format("Expected proxy type ({0}) to be generic", one.GetType()));
		}
		...
		{
			var one = generator.CreateClassProxy(typeof (Empty<string>), new[] { typeof (IEmpty<string, int>) });
		...
		{
			var proxy = generator.CreateClassProxy<Empty<string>>();
		...
		{
			var proxy = generator.CreateClassProxy<Empty<IDictionary<Func<string>, ICollection<Predicate<int>>>>>();
