...
namespace CastleTests.OpenGenerics
{
	...
	public class InterfaceProxyWithTargetInterfaceEmptyInterfaceTestCase : BasePEVerifyTestCase
	{
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface<IEmpty<string>>(new Empty<string>());
			var two = generator.CreateInterfaceProxyWithTargetInterface<IEmpty>(new Empty());
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface<IEmptyClass<object>>(new EmptyClass<object>());
		...
		{
			var one =
				generator.CreateInterfaceProxyWithTargetInterface<IEmptyWithBaseGenericInterfaceConstraint<GenInterfaceImpl<int>>>(
		...
		{
			var one =
				generator.CreateInterfaceProxyWithTargetInterface<IEmptyWithBaseInterfaceConstraint<Empty>>(
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface<IEmptyNew<int>>(new EmptyNew<int>());
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface<IEmptyStruct<int>>(new EmptyStruct<int>());
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface<IEmptyVariant<object, string>>(new EmptyVariant<object, string>());
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface<IEmptyWithBase<int>>(new EmptyWithBase<int>());
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface<IEmpty<string>>(new Empty<string>());
			var two = generator.CreateInterfaceProxyWithTargetInterface<IEmpty<string, int>>(new Empty<string, int>());
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface<IEmpty<string>>(new Empty<string>());
			var two = generator.CreateInterfaceProxyWithTargetInterface<IEmpty<int>>(new Empty<int>());
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface<IEmpty<string>>(new Empty<string>());
			var two = generator.CreateInterfaceProxyWithTargetInterface<IEmpty<string>>(new Empty<string>());
		...
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface(typeof (IEmpty<string>), new[] {typeof (IEmpty)},
			                                                   new Empty<string>());
			Assert.True(one.GetType().IsGenericType, string.Format("Expected proxy type ({0}) to be generic", one.GetType()));
		}
		...
		public void Can_generate_generic_proxy_with_additional_interface_and_mixin()
		{
			...
			var one = generator.CreateInterfaceProxyWithTargetInterface(typeof (IEmpty<string>), new[] {typeof (ISimple)},
		...
		public void Can_generate_generic_proxy_with_mixin()
		{
			var options = new ProxyGenerationOptions();
			options.AddMixinInstance(new Empty());
			var one = generator.CreateInterfaceProxyWithTargetInterface(typeof (IEmpty<string>), new Empty<string>(), options);
			Assert.True(one.GetType().IsGenericType, string.Format("Expected proxy type ({0}) to be generic", one.GetType()));
		}
		...
		[Test(
			Description =
				"We might allow that if we can close base type over primary interface's generic args, but let's wait for someone who actually needs this first."
			)]
		public void Generic_base_interface_proxy_type_not_allowed_open()
		{
			...
			var exception =
				Assert.Throws<ArgumentException>(
					() => generator.CreateInterfaceProxyWithTargetInterface(typeof (IEmpty<string>), new Empty<string>(), options));

			Assert.AreEqual(
				"Type cannot be a generic type definition. Type: " + baseType.FullName + Environment.NewLine +
				"Parameter name: parentType", exception.Message);
		}

		[Test]
		public void Proxy_is_non_generic_if_has_generic_additional_interfaces()
		{
			var one = generator.CreateInterfaceProxyWithTargetInterface(typeof (IEmpty<string>), new[] {typeof (IEmpty<string, int>)},
			                                                   new Empty<string>());
		...
		{
			var proxy = generator.CreateInterfaceProxyWithTargetInterface<IEmpty<string>>(new Empty<string>());
		...
		{
			var proxy =
				generator.CreateInterfaceProxyWithTargetInterface<IEmpty<IDictionary<Func<string>, ICollection<Predicate<int>>>>>(
